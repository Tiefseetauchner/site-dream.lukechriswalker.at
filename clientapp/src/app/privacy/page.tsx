...
export default async function PrivacyPage() {
  const response = (await client().single("privacy").find()) as unknown as PrivacyResponse;

  const privacy = response.data;

  const updatedDate = privacy.updated ? new Date(privacy.updated as string) : null;
  const formattedUpdated =
    updatedDate && !Number.isNaN(updatedDate.getTime()) ? new Intl.DateTimeFormat("en-US", { dateStyle: "long" }).format(updatedDate) : null;
    ...
    <Panel>
      <div className="space-y-4">
        {privacy.content ? (
          ...
          <p className="text-sm text-stone-100">The privacy policy will be available soon. Please check back later.</p>
        )}
        {formattedUpdated && <p className="text-sm font-semibold uppercase tracking-widest text-stone-200">Last updated: {formattedUpdated}</p>}
      </div>
    </Panel>
  );
}

...
export default async function BookDetailPage({ params }: BookPageProps) {
  ...
  const authors = Array.isArray(book.authors) ? (book.authors as BookAuthor[]) : [];
  const links = Array.isArray(book.links) ? (book.links as BookLink[]) : [];
  const actions = links
    .map((link) => {
      const label = link.display_name?.trim() || "Visit link";
      const normalized = label.toLowerCase();
      const variant = ["read online", "get the ebook", "get ebook", "ebook", "e-book"].includes(normalized)
        ? "primary"
  ...
  return (
    <>
      <PageMetadata title={book.title ?? undefined} subtitle={bookDetailMetadata.subtitle} />
      <div className="space-y-8">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,320px)_1fr]">
          ...
          <Panel>
            {book.title && <h2 className="text-3xl font-semibold tracking-wide text-white">{book.title}</h2>}
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-stone-300">
              <Link href={routes.books} className="underline underline-offset-4 hover:text-stone-50">
                Dreams series
              </Link>
              {typeof book.order === "number" && book.order > 0 ? ` · Book ${book.order}` : ""}
            </p>
            {authors.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-lg font-semibold uppercase tracking-widest text-stone-200">Author{authors.length > 1 ? "s" : ""}</h3>
                <ul className="space-y-1 text-base text-stone-100">
                  {authors.map((author) => (
                    <li key={author.slug ?? author.name}>
                      {author.slug ? (
                        <Link href={routes.author(author.slug)} className="underline underline-offset-4 hover:text-stone-50">
                          {author.name}
                        </Link>
                      ) : (
            ...
            {book.content_warnings && (
              <details className="group rounded-lg border border-white/10 bg-black/30 p-3">
                ...
                <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-stone-100">{book.content_warnings}</p>
              </details>
            )}
            {actions.length > 0 && (
              <div className="space-y-3">
                <h3 className="text-lg font-semibold uppercase tracking-widest text-stone-200">Read Yourself!</h3>
